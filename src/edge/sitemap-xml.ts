...
export function buildSitemapXml(siteUrl: string, items: SitemapItem[]): string {
  const statics = STATIC_SITEMAP_PATHS.map(p => `  <url><loc>${escapeXml(canonicalUrl(siteUrl, p))}</loc></url>`);
  const shared = items
